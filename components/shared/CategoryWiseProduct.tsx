...
const CategoryWiseProduct = ({ product }: { product: any }) => {
    ...
    return (
        <div className='border border-gray-300 p-2 bg-white h-full'>
            <h1 className='font-bold capitalize text-center text-lg'>{product.category}</h1>
            <Link href={`/product/${product.id}`}>
                <div className='my-2 flex items-center justify-center bg-gray-100 h-[250px] overflow-hidden object-cover rounded-md'>
                    <Image className='p-8 mix-blend-multiply' src={product.image} width={200} height={200} alt={product.title} />
                </div>
            </Link>
